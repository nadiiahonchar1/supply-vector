...
No semicolons inside string literals or function bodies. The runner splits each file into statements on ; — it does not parse SQL, so a semicolon anywhere other than a statement terminator will break it. Stick to plain DDL (CREATE TABLE, ALTER TABLE, CREATE INDEX); avoid DO $$ ... $$ blocks or PL/pgSQL functions in a migration file.
